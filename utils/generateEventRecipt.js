...
const generateEventReceiptDetails = async (transaction_ref) => {
  try {
    ...
    return {
      ...
      amount: amountPaid,
      reference,
      regNo: registrationNumber || schoolStudent.registrationNumber, // Use from EventPayment or SchoolStudent
      fullName,
      department,
      academicLevel,
      schoolName: schoolInfo.university,
      feeType: feeType || "Event Registration", // Include fee type
      date,
      time,
    };
...
const generateEventReceiptDetailsWithSchool = async (transaction_ref, schoolName = null) => {
  try {
    ...
    const {
      ...
      createdAt,
      studentId,
      schoolName: transactionSchoolName // If you added this to EventTransaction schema
    } = transaction;
...
// Also export the alternative function as a property
